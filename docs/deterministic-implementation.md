...
Status: design proposal with explicitly marked implementation-baseline sections. Verification Policy, the quiescent Wave completion suite, and Slice 3 Task-local Implementation Attempt authority are shipped. Slice 4 bounded retry/escalation is implemented and locally validated on `feat/deterministic-implementation-retry` (issue #39) but is not yet claimed shipped. The bounded Defect-Family Accounting floor described as **P3 implemented baseline** below is implemented and locally validated on `feat/defect-family-closure`; one registered review and its adjudicated repairs are complete and locally verified, but installation, publication, hosted CI, and merge are not claimed. See the [P3 Plan](../.claude/plans/2026-09-08-defect-family-accounting.md) for exact evidence and dispositions. Later items remain proposals. For the complete shipped guarantees this proposal builds on, see [Deterministic core](deterministic-core.md).
...
Some findings stay semantic no matter what. The implemented P3 floor reduces unsupported closure claims without implementing the broader predicate-compilation or diff-scoped re-review proposals.

**P3 implemented baseline — bounded Defect-Family Accounting.** A schema-v2 standalone remediation start accounts for every surviving-critical Finding ID from one authoritative standalone result. It copies those IDs unchanged into repaired or blocking dispositions; separately named repair groups declare root cause, invariant, sibling accounting, selected checks, and Historical RED. Those semantic claims retain `DECLARED` provenance. The registered engine executes each distinct selected operator command once in proper standalone-remediation scope and accepts only fresh parseable JUnit/Vitest report bytes with a normal exit, positive executed count, zero failures, and unchanged candidate authority. Unresolved/out-of-scope criticals or siblings remain valid accounting and block. Zero criticals require explicit `defectFamily: {"kind":"not-required"}` but no manifest or subprocess. The resulting installation label is only `repair-checked`; it does not mint `ResolvedFinding`, prove semantic grouping, or observe a vulnerable historical snapshot.

The repaired boundary requires a **new** report: before critical-check launch, the engine removes the exact old ignored/untracked regular file via no-follow, descriptor-relative unlink anchored to its Linux parent. Touching seeded green bytes cannot pass; writing identical fresh bytes can. Strict critical reset is **Linux-only**; Darwin fails before launch. Zero-critical remediation and Wave behavior are unchanged. Structured reports are limited to **8 MiB** and XML element depth **128**. V2 retained-event reads, append reconciliation/new appends, and CLI inspection are bounded to **12 MiB per encoded event, 64 MiB per encoded journal, 1024 records**, before oversized decoding/JSON parsing. These limits do not cap all Run artifacts or change legacy journal policy. An operator-owned fixed command can still fabricate a valid new report: structured fresh engine observations are not semantic proof.

The install path is versioned and fail-closed: preflight runs before Run Directory creation; opaque assessment authority follows candidate/path/index witnesses into the real Git adapter; the external `done` result carries the adapter's actual nested installation receipt. Completed schema-v1 runs are read-only `historical-unknown` and return their old receipt without reinstalling, while unfinished v1 runs block; missing v2 fields never downgrade. Completed-v2 replay rejects missing/malformed audit arrays with an explicit checkpoint audit-path diagnostic rather than empty-array fallback. This floor does **not** add a full reviewer rerun after each repair, a dual-snapshot executor, or a generalized task runner. The remaining T3 items below are still proposals.

**Historical P3 implementation context (2026-09-08):** that implementation deliberately left `project:verify` at `report.kind: "not-required"` and ineligible for critical P3. Its bootstrap allowance was review/installation by the admitted CLI and loaded Skill 3.1 after external validation; such a bootstrap would be historical-unknown after reload, not its own v2 repair-checked publication. New live P3 operation requires package reload/restart without bypassing runtime admission; source review publication and interrupted/recovered evidence remain immutable.

**Enrollment follow-through (2026-09-09):** the explicitly user-approved idle operator replacement now requires `.loom/completion-reports/verify.junit.xml` from the unchanged root `npm run verify`. The existing Vitest invocation emits that ignored/untracked JUnit file; its counts describe only Vitest, while whole-command normal zero exit also proves compiler and all six smokes passed. Configuration is not passing evidence or a completed live schema-v2 run. This small follow-up precedes priority 4 and does not implement it; see its [separate plan](../.claude/plans/2026-09-09-junit-verification-enrollment.md). The historical P3 Plan and run evidence are not rewritten.
