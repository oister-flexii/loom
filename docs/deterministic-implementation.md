...
Loom's protected `project:verify` remains `report.kind: "not-required"` and ineligible for critical P3; these changes do not enroll a report command. After external validation, this feature may be reviewed/installed by the admitted CLI and loaded Skill 3.1 under their existing protocol. That bootstrap would be historical-unknown after reload, not this feature's own v2 repair-checked publication. New live P3 operation requires package reload/restart without bypassing runtime admission; source review publication and interrupted/recovered evidence remain immutable.
