...
- For schema 2, load the exact P4 guidance in `references/reviewer-protocol-v2/agents/pr-test-analyzer.md` and `references/reviewer-protocol-v2/agents/_shared/wire-contract.md`; no successor requirement applies.
...
Missing tests alone do not establish a blocking consequence. Identify a concrete supported regression risk or explicit acceptance/verification obligation and its delivery consequence. Use the issued impact rubric, not numeric coverage ratings. `truthConfidence` is confidence in the assertion, not its severity.

Emit only the issued JSON payload. Evidence, limits, relevant obligations and testing recommendations belong in the Finding fields; concise nonblocking improvements use advisory with a reason. No separate summary, ratings, positive-observation section or delegation report.
