...
Format `gzip-json-path-base64bytes-v1`: UTF-8 `JSON.stringify` of an array of `{path, base64bytes}` objects, with those two keys in that order, entries ordered exactly as the original inventory, standard padded canonical base64, no whitespace/BOM/trailing newline. Compress using standard Node `gzipSync(bytes, {level: 9})` (zero gzip MTIME, no filename); tests encode twice and compare complete gzip bytes, including metadata, to the stored pack. Reproduction uses original bytes from `loadReviewerV1Golden`, not parsed/reserialized artifact JSON. Exact gzip reproduction is tested against the repository's Node/zlib toolchain; other zlib versions can choose a different lossless deflate stream and would require an explicit storage-only manifest update.
