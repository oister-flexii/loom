...
import { parseContextPacket, type ContextPacket } from "./context-packets";
...
export function parseContextProjectionArguments(args: readonly string[]): DomainResult<ContextProjectionInput, string> {
  ...
  const allowed = ["--packet", "--request", "--digest", "--role", "--skill", "--section", "--file", "--offset", "--limit"];
...
function fileText(packet: ContextPacket, path: string): DomainResult<string, string> {
  ...
  if (file.kind !== "text" || typeof file.content !== "string") return failed("source file is binary or absent; no text projection available");
...
function sectionText(packet: ContextPacket, label: string): DomainResult<string, string> {
...
function project(packet: ContextPacket, selection: Selection): DomainResult<unknown, string> {
