...
`preverify` checks tools and local executable resolution, not exact Node/Bun versions. CI pins and checks those versions on `ubuntu-24.04`, checks out full history with `fetch-depth: 0`, installs both frozen graphs, preserves verification failure through Bash `pipefail`, and always attempts log upload. Full history is a real suite prerequisite because the committed calibration corpus names revisions available through remote refs. The CI job has a 30-minute budget; the runtime command independently has a 30-minute timeout.

Recorded local versions include Pi 0.84.2 and TypeScript 5.9.3 from the existing locks. macOS 13+ remains the existing runtime support contract, but this work has **no macOS test evidence** and no green GitHub-hosted CI result; the completed red push/PR evidence is recorded below. Report existing platform-dependent skips, such as the Darwin-only filesystem case on Linux; do not claim universal zero skips. Non-root execution keeps permission-denial tests meaningful.
...
| CI | `.github/workflows/ci.yml`, `engine/tests/verification-ci.test.ts` | PR/branch/tag triggers, full-history checkout, exact runtimes, two frozen installs, local executables, non-root Linux tools, same root command, failure-preserving log upload. Hosted execution remains separate evidence. |
...
Closing post-Plan document checks, exact source/document byte inventory, support-path roster, log hashes and before/after preservation evidence are recorded in `/tmp/loom-verification-adjudicated-remediation.md`. The sole new support path beyond the original registered 29-path scope was `engine/tests/command-preflight-contract.test.ts` (30 feature paths before hosted execution). The existing manifest property test was executed, not modified. These records are verification evidence, not an installed verified index or a new formal review result.

### Hosted CI completion closure

Both Linux verification runs for exact head `d4f54d494fa52600fcf008ba0402791b94c4fa8d` failed with the same bounded signature: push run [34208566352](https://github.com/peterstorm/loom/actions/runs/34208566352) and PR run [34208566507](https://github.com/peterstorm/loom/actions/runs/34208566507) each reported **5 calibration-test failures caused by unavailable historical revisions** plus **1 top-level source-domain acyclicity timeout at the unchanged 15-second test budget**. Vitest therefore stopped the canonical command before all six smokes. This records red hosted evidence; it is not a semantic feature failure and is not relabeled green.

A depth-1 single-branch clone at `d4f54d4` reproduced exactly the five history-dependent failures. A fresh full clone checked out on `main` resolved every corpus revision (`4927f0c`, `cc0c592`, `907771d`, `8ce7c58`, `ec2cc90`, `0710b76`, `592fa8d`) through `origin/feat/architecture-panel-mode-plan`; that branch tip also matched the GitHub remote. Fetching all remote heads to unshallow the reproduction clone made all four affected files pass (**40/40**) with no test skip or timeout-budget increase. This proves `actions/checkout` `fetch-depth: 0` is sufficient; no corpus check is skipped or weakened.

The graph check now visits each node at most once per root, retaining runtime and type-only import edges and the exact existing three-member legacy allowlist. Finite controls cover a self-cycle, disjoint cycles, and acyclic nodes both reachable from and dependent on a cycle. A bounded property compares exact cycle members with brute-force simple-path enumeration on small graphs; a 20-layer diamond asserts the O(V·E) edge-visit bound without a wall-clock assertion.

The final cumulative feature roster is the previous 30 paths plus exactly one support path: `engine/tests/core/orchestration-contract-acyclic.test.ts` (**31 total**). This closure modifies six existing feature paths (`.github/workflows/ci.yml`, `engine/tests/verification-ci.test.ts`, `README.md`, `docs/operations.md`, `docs/workflows.md`, and this Plan) and that sole new support path. It adds no production source, allowlist, dependency, manifest, State File, Run, receipt, stash, index, commit, or publication change.

Measured local closure on the unstaged final source: the focused workflow/command/calibration/runbook/graph set passed **6 files / 75 tests**; typecheck passed with only the same 16 printed upstream raw-TypeScript unused exclusions; full-tier lint passed both changed TypeScript tests with zero violations/errors. Distill apply mode retained the bounded scan, bounded brute-force oracle, and explicit controls as essential evidence and applied no additional move; the same **10/10** workflow/graph assertions remained green. The unfiltered root `npm run verify` then exited 0 with **249 files / 7,570 tests passed / 1 existing Darwin-only skip**, followed by all six smokes: architecture panel **22/22**, refutation panel **19/19**, standalone review PASS, orchestration façades PASS, Pi resources PASS, and TaskGraph validation **23/23**. This local green run neither skips the exposed tests nor raises their 15-second budget; it is not the pending hosted rerun.
